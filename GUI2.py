...
class TestingTool:
    ...
    def create_year_comboboxes(self):
        self.create_year_combobox("Select Start Year:", "start_year_var", row=1)
        self.create_year_combobox("Select End Year:", "end_year_var", row=2)

    def create_year_combobox(self, label_text, var_name, row):
        ttk.Label(self.tab1, text=label_text).grid(row=row, column=0, pady=10, padx=10, sticky="e")

        years = [str(year) for year in range(2000, 2017)]  # Years from 2000 to 2016
        setattr(self, var_name, tk.StringVar(value="2007"))
        year_combobox = ttk.Combobox(self.tab1, textvariable=getattr(self, var_name), values=years, state="readonly")
        year_combobox.grid(row=row, column=1, pady=10, padx=10, sticky="w")
    ...
    def create_input_entries(self):
        self.start_year_var.set("2007")
        self.end_year_var.set("2011")
